...
#define IMAGE_PATH "C:\\Users\\dbintley\\OneDrive - University of Plymouth\\Year 2\\ROCO224\\Image Code\\img1.png"
...
//    drawPath.insert(drawPath.end(), outlinePath.begin(), outlinePath.end());
    ...
    cout<<"\tmoveToTarget("+to_string(drawPath.front().to.x*10)+","+to_string(drawPath.front().to.y*10)+",restZ,stepperDelay);"<<endl;
    for(auto i:drawPath)
    {
        if(i.type==draw)
        {
            cout<<"\tmoveToTarget("+to_string(i.to.x*10)+","+to_string(i.to.y*10)+",drawZ,stepperDelay);"<<endl;
        }
        else
        {
            cout<<"\tmoveToTarget(currentX,currentY,travelZ,stepperDelay);"<<endl;
            cout<<"\tmoveToTarget("+to_string(i.to.x*10)+","+to_string(i.to.y*10)+",travelZ,stepperDelay);"<<endl;
            cout<<"\tmoveToTarget(currentX,currentY,drawZ,stepperDelay);"<<endl;
        }
    }

    cout<<"\tmoveToTarget("+to_string(drawPath.back().to.x*10)+","+to_string(drawPath.back().to.y*10)+",restZ,stepperDelay);"<<endl;
